...
to setup-world
  ...
  set clouds-amount-at-start 25
  set CO2-amount-at-start 50
  add-start-clouds
  add-start-CO2
end
...
to add-start-clouds           
  ...
  create-clouds clouds-amount-at-start
  [
    set cloud-speed speed
    set cloud-id id
    setxy x + random 30 - 4
          y + 2.5 + random-float 4 - random-float 2
    set color white
    set size 2 + random 2
    set heading 90
  ]
...
to update-rays-heading
  if (ticks mod 1000 = 0) [
    set rays-heading 160
  ]
  if (ticks mod 100 = 0 and ticks mod 1000 != 0) [
    set rays-heading rays-heading - 5
  ]
end
...
to add-start-CO2  ;; randomly adds 25 CO2 molecules to atmosphere
  let sky-height sky-top - earth-top
  create-CO2s CO2-amount-at-start [
    set color green
